...
class InvalidSpecE2ETest {
    ...
    void userWithoutSpecReportsInvalidSpec() {
        ...
        user.setMetadata(new ObjectMetaBuilder().withName("invalid-spec-user").withNamespace(NS).build());
    ...
    void bucketWithoutSpecReportsInvalidSpec() {
        ...
        bucket.setMetadata(new ObjectMetaBuilder().withName("invalid-spec-bucket").withNamespace(NS).build());
    ...
    void userReferencingIncompleteBackendReportsInvalidSpec() {
        ...
        backend.setMetadata(new ObjectMetaBuilder().withName("incomplete-backend").withNamespace(NS).build());
        ...
        S3User user = new S3User();
        user.setMetadata(new ObjectMetaBuilder().withName("incomplete-backend-user").withNamespace(NS).build());
    ...
    void bucketWithMissingReferencesReportsInvalidSpec() {
        ...
        S3Bucket bucket = new S3Bucket();
        bucket.setMetadata(new ObjectMetaBuilder().withName("missing-ref-bucket").withNamespace(NS).build());
        bucket.setSpec(spec);
        client.resources(S3Bucket.class).inNamespace(NS).resource(bucket).create();
